...
wiki:
  ...
  primary_diagram:
    ...
    accDescr: >-
      ...
      never started, and the failed candidate quarantined.
  status: active
...
> consumes the old cutover authority, and guards future backend mutations.
> Backend [`PR #483`](https://github.com/ramideltoro/nutsnews-backend/pull/483),
> merge `5531014000f52fd6101f8617463d5f2c887d0788`, repairs the forward
> publication contract and retry identity. Both are source-only: no deployment
> or replay occurred. The worker deploy guard and infra verifier remain
> unfinished and frozen.

> **Freeze.** Static runtime status can still show shadow/write-disabled and
> cannot prove owner or rollback completion. Use only read-only status, logs,
> and queue inspection. Do not run Ansible or any generic runtime mutation,
> replay, reconciliation, further Grafana mutation, synthetic drills, Runtime 1/fetcher v2, or
> web merge. Rollback is complete: never rerun cutover, rollback, or finalize.
> The failed Runtime 0.x candidate is disqualified and quarantined. See the [central incident
> evidence](https://github.com/ramideltoro/nutsnews-infra/issues/474#issuecomment-5153075316).
...
The original source defaults were shadow-first. They are implementation
history, not the current authoritative control row:
...
digest return the recorded response. Regenerated delivery identifiers do not
change that digest; a different article command still conflicts. PR #483 also
requires exactly one real HTTP(S) article URL, `published` state, the protected
five-language scope, and exact one-row confirmation before recording success.
That repair is merged source but is not deployed. Do not enable
...
These are fixed workflow capabilities, but every mutation is frozen. Generic runtime `rollback` is not the cutover rollback. Use only read-only evidence; the completed rollback must not be rerun.

## Zero-Consumer Readiness And Recovery

Every consuming service treats its contracted main queue as part of readiness. The scheduler is producer-only and does not require a consumer. `/ready` returns unhealthy when the service has zero active consumers. Backend `status` reports the HTTP readiness result and RabbitMQ consumer count for each service, while a main-queue `queue-inspect` fails when a declared consumer queue has a healthy RabbitMQ snapshot with zero consumers.
...
The following restart and deployment procedures are historical and frozen during the post-rollback hold. Outside the freeze, the protected restart path applies when the reviewed image and configuration are still correct and only the live consumer was lost:
...
Those pre-cutover paths preserved writes false and legacy ownership. The generation 5 finalize artifact now proves stable shadow. Grafana remains infra-owned, but Grafana apply is frozen.
...
| [`ramideltoro/nutsnews-backend#482`](https://github.com/ramideltoro/nutsnews-backend/pull/482) | Consume historical authority and guard backend mutations | `510b775d7962e2e66d430fb6d458c3c88d60cdd3` |
| [`ramideltoro/nutsnews-backend#483`](https://github.com/ramideltoro/nutsnews-backend/pull/483) | Repair publication semantics and stable command idempotency | `5531014000f52fd6101f8617463d5f2c887d0788` |
