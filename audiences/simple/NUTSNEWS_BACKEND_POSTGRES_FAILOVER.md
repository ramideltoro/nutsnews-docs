---
title: NutsNews Backend PostgreSQL Primary And Supabase Hot Standby
wiki:
  source_route: /technical/nutsnews-backend-postgres-failover/
  simple_route: /simple/nutsnews-backend-postgres-failover/
  primary_diagram:
    file: diagrams/NUTSNEWS_BACKEND_POSTGRES_FAILOVER.mmd
    accTitle: "Backend PostgreSQL primary and Supabase hot standby readiness"
    accDescr: "Maps backend PostgreSQL as production primary, the existing Supabase standby target, standby manifests, reconciliation, relay, failover gates, and protected promotion sequence."
  status: active
...
- the locked `nutsnews-standby-relay` OS user runs a snapshot-apply relay on the
  backend host;
- each run reads the backend source tables over loopback, upserts changed rows,
  deletes target-only rows, advances sequences, and writes outbound to the
  existing production Supabase direct PostgreSQL endpoint;
...
- the apply path upserts source rows, deletes target-only rows, and validates
  parity after each sync run;
