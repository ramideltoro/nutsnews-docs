---
title: NutsNews Backend Drift Check
wiki:
  source_route: /technical/nutsnews-backend-drift-check/
  simple_route: /simple/nutsnews-backend-drift-check/
  primary_diagram:
    file: diagrams/NUTSNEWS_BACKEND_DRIFT_CHECK.mmd
    accTitle: "Backend drift check and reporting"
    accDescr: "Read-only check compares live backend host state with repo baseline and reports drift before risky changes."
  status: active
