---
title: NutsNews Infra Operations Platform
wiki:
  source_route: /technical/nutsnews-infra-operations-platform/
  simple_route: /simple/nutsnews-infra-operations-platform/
  primary_diagram:
    file: diagrams/NUTSNEWS_INFRA_OPERATIONS_PLATFORM.mmd
    accTitle: "Ops platform control model"
    accDescr: "Shows GitOps flow from repo changes through protected apply, VPS health reporting, and platform operations."
  status: active
...
The `Infrastructure Checks` Ansible lint job keeps two Python cache layers: `actions/setup-python` caches pip downloads from `.github/requirements/ansible-lint.txt`, and a separate `.venv/ansible-lint` cache stores the installed tooling keyed by runner OS, resolved Python version, and that same requirements file. The shared `ansible/requirements.yml` now pins Galaxy collection versions so deployment workflows can safely cache collections by that file hash; `Infrastructure Checks` still installs collections fresh unless a later PR adds a collection cache to that job.

Workflow Safety also validates remote GitHub Actions `uses:` references with a generic pinning check. Local actions such as `./...` remain allowed, but third-party and official actions must use full commit SHAs unless a narrow central allowlist documents the exception and reason. Guardrail tests that care about sensitive actions now check the approved action identity plus immutable pinning instead of freezing a specific historical action commit.
