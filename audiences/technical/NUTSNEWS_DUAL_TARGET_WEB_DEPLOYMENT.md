---
title: NutsNews Dual-Target Web Deployment
wiki:
...
### Operations Closeout Evidence

Issue [nutsnews #398](https://github.com/ramideltoro/nutsnews/issues/398)
closed the post-cutover operations update on July 22, 2026 UTC.

The first VPS-primary release after the docs/monitor closeout used application
source commit `521fcababfbaff54785c6b5cf5995dc7f9cf62a1`, build
`29885890520-1`, and immutable image
`ghcr.io/ramideltoro/nutsnews@sha256:46ced037cbd11bbdc1a80cb1c520e6eadfde8128f3bd5e5f2edca5479b72ee7f`.
The app PR was
[ramideltoro/nutsnews#403](https://github.com/ramideltoro/nutsnews/pull/403),
and the production GitOps promotion PR was
[ramideltoro/nutsnews-infra#356](https://github.com/ramideltoro/nutsnews-infra/pull/356).

Release and verification runs:

| Stage | Run |
| --- | --- |
| App `main` container image | `ramideltoro/nutsnews` run `29885890520` |
| Staging deploy | `ramideltoro/nutsnews-infra` run `29886778962` |
| Staging qualification | `ramideltoro/nutsnews-infra` run `29886833168` |
| Production promotion | `ramideltoro/nutsnews-infra` run `29886896650` |
| Protected VPS apply | `ramideltoro/nutsnews-infra` run `29886979754` |
| Vercel secondary production | `ramideltoro/nutsnews` run `29887269539` |
| Manual Cloudflare cache purge | `ramideltoro/nutsnews` run `29887566515` |
| Live cache observability | `ramideltoro/nutsnews` run `29887610632` |

After the protected apply and Vercel secondary deploy, these live checks passed:

| Surface | Expected result |
| --- | --- |
| `https://www.nutsnews.com/api/runtime-config` | `deploymentTarget=production-vps`, source `521fcababfbaff54785c6b5cf5995dc7f9cf62a1`, build `29885890520-1` |
| `https://www.nutsnews.com/healthz` | `ok=true`, target header `vps`, same source and build |
| `https://www.nutsnews.com/readyz` | `ok=true` |
| `https://vps.nutsnews.com/healthz` | Direct VPS health is `ok=true` for the same source and build |
| `https://nutsnews.vercel.app/healthz` | Vercel secondary health is `ok=true`, `deploymentTarget=vercel-production`, same source and build |
| `https://www.nutsnews.com/articles/sitemap/0.xml` | HTTP `200` after purging the stale Cloudflare `500` edge object |

The final DNS failover controller status was:

| Field | Value |
| --- | --- |
| Controller | `nutsnews-production-vps-primary` |
| `activeDnsTarget` | `vps` |
| `dnsWritesEnabled` | `true` |
| `lastHealthStatus` | `healthy` |
| `manualLock` | `false` |
...
| `failureThreshold` | `3` |
| `recoveryThreshold` | `1` |
| `minDnsUpdateIntervalSeconds` | `60` |
| Managed records | `nutsnews.com CNAME`, `www.nutsnews.com CNAME` |
| Primary target | `vps.nutsnews.com` |
| Secondary target | `cname.vercel-dns.com` |

External monitors are:

| Monitor ID | Name | URL |
| --- | --- | --- |
| `4519549` | `nutsnews.com` | `https://www.nutsnews.com` |
| `4637957` | `vps.nutsnews.com/health` | `https://vps.nutsnews.com/health` |
| `4705764` | `nutsnews VPS primary app healthz` | `https://vps.nutsnews.com/healthz` |
| `4705765` | `nutsnews Vercel secondary healthz` | `https://nutsnews.vercel.app/healthz` |
| `4705766` | `nutsnews DNS failover controller healthz` | `https://nutsnews-dns-failover.nutsnews.workers.dev/healthz` |

The live cache observability workflow ran against
`https://www.nutsnews.com` and passed. It may warn when Cloudflare consumes
targeted CDN control headers or when dynamic routes do not produce `HIT`
samples, but a green run means the public cache policy did not regress. The
post-release Sentry sweep found zero unresolved issues first seen after
