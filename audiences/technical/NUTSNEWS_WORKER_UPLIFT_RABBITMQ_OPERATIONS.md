---
title: NutsNews Worker-Uplift RabbitMQ Operations
wiki:
  source_route: /technical/nutsnews-worker-uplift-rabbitmq-operations/
  simple_route: /simple/nutsnews-worker-uplift-rabbitmq-operations/
  primary_diagram:
    file: diagrams/NUTSNEWS_WORKER_UPLIFT_RABBITMQ_OPERATIONS.mmd
    accTitle: "RabbitMQ operations guardrails and smoke/canary flow"
    accDescr: "Shows protected apply, drift checks, smoke validation, private canary controls, and read-only health-report evidence for the worker-uplift broker."
  status: active
...
`RABBITMQ_MONITORING_USERNAME` and `RABBITMQ_MONITORING_PASSWORD`. That
identity is least-privilege: it can write to and read from only the isolated
canary route and cannot configure RabbitMQ resources or access worker-uplift
production route queues.
