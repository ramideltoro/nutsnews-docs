---
title: Cloudflare KV Worker State for NutsNews
wiki:
  source_route: /technical/cloudflare-kv-worker-state/
  simple_route: /simple/cloudflare-kv-worker-state/
  primary_diagram:
    file: diagrams/CLOUDFLARE_KV_WORKER_STATE.mmd
    accTitle: "KV state flow for shard dedupe, run state, and status debugging"
    accDescr: "Shows optional KV binding path in the shard worker, what is cached in KV, when writes happen, and the `/kv-status` and rollout checks."
  status: active
